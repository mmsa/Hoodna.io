...
function SocialMark({ kind }: { kind: LinkHostKind }) {
  if (kind === 'facebook') {
    return (
  ...
  return <ExternalLink className="h-10 w-10 text-white" />
}
...
function SocialHero({ kind }: { kind: LinkHostKind }) {
  ...
  return (
    <div
      className="relative flex aspect-[16/9] w-full flex-col items-center justify-center gap-3"
      style={{
        background: `linear-gradient(145deg, ${theme.bg} 0%, ${theme.bgEnd} 100%)`,
      }}
    >
      ...
      <div className="relative flex h-14 w-14 items-center justify-center rounded-full bg-white/95 shadow-lg">
        <Play className="ml-0.5 h-7 w-7 fill-current text-black" />
      </div>
      <p className="relative text-sm font-semibold tracking-wide text-white/95">
...
export function LinkPreviewCard({
  ...
  const isSocialFallback =
    !preview.image &&
    (kind === 'facebook' || kind === 'tiktok' || kind === 'instagram' || kind === 'twitter')
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      className={cn(
        'mt-3 block overflow-hidden rounded-[16px] border border-border bg-card shadow-sm transition hover:shadow-md',
        className
      )}
    >
      {preview.image ? (
