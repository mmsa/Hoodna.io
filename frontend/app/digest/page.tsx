    ...
    <main className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {digest.isError ? <Card><CardContent className="p-8 text-center">We could not load your digest.</CardContent></Card> : !digest.data ? <Card><CardContent className="p-8 text-center">Your first digest will appear here after a week of activity.</CardContent></Card> : (
          <>
            <DigestPosts title="Popular posts" icon={<MessageCircle />} items={digest.data.popular_posts} />
            <DigestPosts title="Announcements" icon={<Bell />} items={digest.data.announcements} />
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><Building2 className="h-5 w-5" />New businesses</CardTitle></CardHeader><CardContent className="space-y-2">{digest.data.new_businesses.length ? digest.data.new_businesses.map((business) => <Link className="block rounded-lg border p-3 hover:bg-gray-50" href={`/businesses/${business.slug}`} key={business.id}>{business.name}<span className="ml-2 text-sm text-gray-500">{business.category}</span></Link>) : <p className="text-sm text-gray-500">No new businesses this week.</p>}</CardContent></Card>
          </>
        )}
      </div>
    </main>
...
function DigestPosts({ title, icon, items }: { title: string; icon: React.ReactNode; items: DigestSummary["popular_posts"] }) {
