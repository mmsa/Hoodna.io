...
async def verify_document_with_llm(
    ...
    try:
        ...
        if document_type == "NATIONAL_ID":
            prompt = f"""Analyze this National ID document (may be in Arabic or English) for user "{user_name}" (email: {user_email}){compound_context}.
...
1. Is this a valid, authentic National ID document?
2. Is the document clear, readable, and not tampered with?
3. Name Matching: Does the name on the ID match or closely match "{user_name}"?
...
4. Is the document current and not expired?
5. Are there any signs of forgery, tampering, or manipulation?
...
{{
  ...
  "reasoning": "detailed explanation including name comparison and why it matches/doesn't match",
  "extracted_info": {{"name": "...", "id_number": "...", "expiry_date": "..."}}
}}"""
        else:  # CONTRACT
            prompt = f"""Analyze this Contract or proof of residency document (may be in Arabic or English) for user "{user_name}" (email: {user_email}){compound_context}.

IMPORTANT: 
- The document may be in Arabic. You MUST read and understand Arabic text if present.
- Names do NOT need to be 100% identical - they should be "close enough" to be the same person.
- Address matching: The address should AT LEAST mention the compound name "{compound_name}" somewhere in the document.
...
{{
  ...
  "issues": ["list", "of", "issues"],
  "recommendation": "APPROVE" or "REJECT" or "REQUEST_MORE_DETAILS",
  "reasoning": "detailed explanation including name and address comparison, mentioning if compound name was found",
  "extracted_info": {{"property_address": "...", "contract_date": "...", "parties": "..."}}
}}"""
